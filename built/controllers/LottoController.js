...
class LottoController {
    ...
    #lottoMachine;
    #lottoChecker;
    constructor(lottoConfig, prizeConfig, inputView, outputView, lottoMachine, lottoChecker) {
    ...
    async start() {
        while (true) {
            try {
                ...
                this.#outputView.printNewLine();
                const winningLotto = new WinningLotto(this.#lottoConfig, winningNumbers, bonusNumber);
                const stats = this.#lottoChecker.calculateStats(lottos, winningLotto);
                this.#outputView.printWinningStatistic(stats);
                const lottoResult = new LottoResult(this.#prizeConfig, purchaseMoney, stats);
